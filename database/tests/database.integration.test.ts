...
describe("PostgreSQL 18 and pgvector migrations", () => {
  ...
  it("preserves dependent work safely when upstream questions, lessons, or source selection change",async()=>{
    ...
    expect(revisions[0]?.snapshot).toMatchObject({title:"合成生命周期任务",questionSet:{centralQuestion:"新问题？"},sourceSelection:{sourceVersionIds:[versionId]},lessonDesign:lesson});
  ...
  it("gates approval and creates an immutable browser-export manifest",async()=>{
    ...
    const source=await sql<{id:string}[]>`select sv.id from core.source_versions sv join core.sources s on s.id=sv.source_id where s.scope='catalog' and sv.rights_state='verified_reusable' and sv.verification_state='verified' order by sv.created_at limit 1`;await sources.select(scope,task.id,0,[source[0]!.id]);
    const revision=await sql<{id:string}[]>`select id from core.task_revisions where task_id=${task.id} and reason='teacher_confirmed' order by revision_no desc limit 1`;await sql`insert into core.verification_runs(workspace_id,task_id,run_kind,input_lock_version,status,started_at,completed_at,summary) values (${workspaceA},${task.id},'design_audit',1,'succeeded',now(),now(),${sql.json({pass:true})})`;
    const approved=await workflow.decide(scope,task.id,{kind:"approve",taskRevisionId:revision[0]!.id},1,"approval-decision-create");expect(approved).toMatchObject({kind:"approve",lockVersion:2});
    const queued=await workflow.enqueueExport(scope,task.id,{format:"pdf",fileName:"合成教案.pdf"},2,"approval-export-create");const lease=crypto.randomUUID();await sql`update ops.jobs set status='running',attempts=1,lease_token=${lease},lease_expires_at=now()+interval '30 seconds',locked_by='synthetic-worker' where id=${queued.jobId}`;const completed=await sql<{id:string|null}[]>`select ops.complete_export(${queued.jobId},${lease},${queued.exportId},2) as id`;expect(completed[0]?.id).toBeTruthy();const manifest=await workflow.exportManifest(scope,task.id,queued.exportId);expect(manifest).toMatchObject({format:"pdf",fileName:"合成教案.pdf",revisionId:completed[0]!.id});expect(JSON.stringify(manifest)).not.toMatch(/workspace|server|path/i);
