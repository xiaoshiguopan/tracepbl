import { fakePriceProfile, generationCost, type PriceProfile } from "@tracepbl/domain";
...
type HandlerOptions = { priceProfile?: PriceProfile; sql: Database; jobs: JobRepository; provider: AiProvider; checkpointer: PostgresSaver; urlFetchEnabled: boolean; actualCostMicros: (inputTokens: number, outputTokens: number) => number };
...
export function createHandlers(options: HandlerOptions): Record<string, JobHandler> {
  return {
    ...
    embedding: (job, signal) => embedSource(options.sql, options.provider, job, signal, options.priceProfile ?? fakePriceProfile()),
...
async function reserveEmbedding(sql: Database, job: JobRow, tokens: number, calls: number, profile: PriceProfile) {
  await sql`select ops.reserve_embedding(${job.id},${job.leaseToken!},${tokens},${calls},${sql.json(profile)})`;
}
async function beginUsage(sql: Database, job: JobRow, kind: string, provider: AiProvider, profile: PriceProfile) {
  if (provider.execution !== profile.mode) throw new WorkerFailure("AI_NOT_CONFIGURED");
  await sql`select ops.begin_usage(${job.id},${job.leaseToken!},${kind},${sql.json(profile)})`;
}
async function settleEmbedding(sql:Database,job:JobRow,calls:number,tokens:number){await sql`select ops.settle_embedding(${job.id},${job.leaseToken!},${calls},${tokens})`;}

async function embedSource(sql: Database, provider: AiProvider, job: JobRow, signal: AbortSignal, prices: PriceProfile) {
  ...
  if(!source[0]) throw new WorkerFailure("RESOURCE_NOT_FOUND"); const chunks=chunkSource(source[0].content_text); const predicted=chunks.reduce((sum,item)=>sum+Buffer.byteLength(item.contentText,"utf8"),0); await reserveEmbedding(sql,job,predicted,Math.ceil(chunks.length/64),prices);
  ...
  await beginUsage(sql,job,"embedding",provider,prices);
