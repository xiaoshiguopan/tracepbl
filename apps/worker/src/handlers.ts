...
async function embedSource(sql: Database, provider: AiProvider, job: JobRow, signal: AbortSignal) {
  ...
  const profile=await sql<{id:string}[]>`select id from rag.embedding_profiles where provider='zhipu' and model='embedding-3' and dimensions=1024 and distance='cosine' and chunker_version='chunk-v1' and active order by created_at desc limit 1`; if(!profile[0]) throw new WorkerFailure("INVALID_STATE");
