...
function RelationCard({ relation, onKind, onRemove }: { relation: EvidenceRelation; onKind: (kind: RelationKind) => void; onRemove: () => void }) {
...
function QuestionZone({ claim, index, children, editing, onEdit, onChange, onGap }: { claim: EvidenceMapDraft["claims"][number]; index: number; children: ReactNode; editing: boolean; onEdit: () => void; onChange: (value: string) => void; onGap: () => void }) {
  const { setNodeRef, isOver } = useDroppable({ id: claim.id, data: { type: "claim", claimId: claim.id } });
  return <article ref={setNodeRef} className={`question-zone ${isOver ? "is-over" : ""}`}><header><span>{index + 1}</span><div>{editing ? <input value={claim.text} autoFocus onChange={(event) => onChange(event.target.value)} /> : <h2>{claim.text}</h2>}</div><IconButton icon={editing ? "check" : "edit"} label={editing ? `完成修改问题${index + 1}` : `修改问题${index + 1}`} onClick={onEdit} /></header>{children || <div className="zone-empty"><p>{claim.gapAccepted ? "此处暂时保留为证据缺口。" : "将史料拖入这里，或暂时标记证据缺口。"}</p><button className="ui-button quiet" type="button" onClick={onGap}>{claim.gapAccepted ? "取消缺口" : "标记暂时缺口"}</button></div>}</article>;
}
...
export function EvidenceMapPage({ taskId = "demo-tang-45m", onBack, onReturnContext, onReturnQuestion, onReturnSources, onNext = () => undefined }: { taskId?: string; onBack: () => void; onReturnContext: () => void; onReturnQuestion: () => void; onReturnSources: () => void; onNext?: () => void }) {
  ...
  const changeClaim = (claimId: string, text: string) => {
    ...
    if (index >= 0) {
      ...
      void loadQuestionDraft(storageKey).then((stored) => stored && saveQuestionDraft(storageKey, { ...stored, subQuestions: next, confirmed: false }));
    }
  };
  ...
  const confirm = () => {
    ...
    setDraft(next);
    void saveEvidenceDraft(storageKey, next).then(onNext, () => setSaveState("failed"));
  };
  ...
  return <WorkbenchShell currentStep={3} reachedStep={3} currentLabel="组织证据" nextLabel="设计活动" onBack={onBack} onNavigateStep={(step) => step === 0 ? onReturnContext() : step === 1 ? onReturnQuestion() : step === 2 ? onReturnSources() : undefined}>
    <main className="context-main evidence-main" id="main-content">
      ...
      {loading ? <div className="context-skeleton" aria-busy="true"><span /><span /></div> : <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd} onDragCancel={() => setActiveTitle("")}><div className="evidence-board"><section className="question-zones" aria-label="子问题与证据">{draft.claims.map((claim, index) => { const relations = draft.relations.filter((relation) => relation.claimId === claim.id); return <QuestionZone key={claim.id} claim={claim} index={index} editing={editingClaim === claim.id} onEdit={() => setEditingClaim((value) => value === claim.id ? null : claim.id)} onChange={(text) => changeClaim(claim.id, text)} onGap={() => setDraft((current) => ({ ...current, claims: current.claims.map((item) => item.id === claim.id ? { ...item, gapAccepted: !item.gapAccepted } : item) }))}>{relations.length ? <ul>{relations.map((relation) => <RelationCard key={relation.id} relation={relation} onKind={(kind) => setDraft((current) => updateEvidenceRelation(current, relation.id, { kind, reason: relation.reason }))} onRemove={() => removeRelation(relation)} />)}</ul> : null}</QuestionZone>; })}</section><aside ref={setShelfRef} className={`source-tray ${shelfOver ? "is-over" : ""}`}><header><div><h2>史料架</h2><p>拖入问题可添加；将问题中的史料拖回这里可移除。</p></div><button className="ui-button secondary compact" type="button" onClick={onReturnSources}>调整史料</button></header><div>{selectedIds.map((id) => <SourceShelfCard key={id} sourceId={id} claims={draft.claims} onAdd={(claimId) => addToClaim(id, claimId)} />)}</div></aside></div><DragOverlay>{activeTitle ? <div className="drag-overlay">{activeTitle}</div> : null}</DragOverlay></DndContext>}
      <PageActionBar status={summary.ready ? `${summary.relationCount} 条证据关系已覆盖全部问题` : "仍有问题缺少证据"} detail={summary.ready ? "可继续设计课堂活动" : "为每个问题加入史料，或明确标记暂时缺口"}><button className="ui-button primary" type="button" disabled={!summary.ready} onClick={confirm}>设计课堂活动</button></PageActionBar>
    </main>
  </WorkbenchShell>;
}
