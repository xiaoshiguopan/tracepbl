...
export type TeachingPackExport = { context: TeachingContextDraft; input: FinalReviewInput; fileName: string };
...
const selectedSources = (pack: TeachingPackExport) => sourceFixture.filter((source) => pack.input.selectedSourceIds.includes(source.id));
const sourceNames = (ids: string[]) => ids.map((id) => sourceFixture.find((source) => source.id === id)?.title || id).join("、");
const activityRange = (pack: TeachingPackExport, index: number) => {
  const start = pack.input.lesson.activities.slice(0, index).reduce((total, activity) => total + activity.minutes, 0);
  return `${start}—${start + pack.input.lesson.activities[index].minutes} 分钟`;
};
...
export async function exportTeachingPackDocx(pack: TeachingPackExport) {
  ...
  const children = [
    ...
    heading("二、探究问题", HeadingLevel.HEADING_1), paragraph(pack.input.question),
    heading("三、史料目录与解读", HeadingLevel.HEADING_1),
    ...sources.flatMap((source, index) => [heading(`${index + 1}. ${source.title}`, HeadingLevel.HEADING_2), paragraph(`${source.nature}｜${source.institution}｜${source.period}`), paragraph(`定位：${source.locator}`), paragraph(`材料节选：${source.excerpt}`), paragraph(`材料释义：${source.meaning}`), paragraph(`可论证什么：${source.interpretation}`), paragraph(`使用边界：${source.limitation}`), new Paragraph({ children: [new ExternalHyperlink({ link: source.url, children: [new TextRun({ text: "查看公开来源", color: "7A3E27", underline: {} })] })], spacing: { after: 120 } })]),
    heading("四、课堂活动", HeadingLevel.HEADING_1),
    ...pack.input.lesson.activities.flatMap((activity, index) => [heading(`${index + 1}. ${activity.title}（${activityRange(pack, index)}）`, HeadingLevel.HEADING_2), paragraph(`学生动作：${activity.studentAction}`), paragraph(`使用史料：${sourceNames(activity.sourceIds)}`), paragraph(`预期成果：${activity.evidenceProduct}`), paragraph(`可能卡点：${activity.difficulty}`), paragraph(`教师引导：${activity.scaffold}`)]),
    ...
    heading("六、使用说明", HeadingLevel.HEADING_1), paragraph("本教学包为可试教候选。短引、来源定位与链接用于课堂备课；请按各来源页面标注的权利边界使用，不包含馆藏图片或长篇说明。"), paragraph("史证工坊公开演示数据保存在当前浏览器；前端展示不代表真实安全授权。"),
  ];
...
export async function exportTeachingPackPdf(pack: TeachingPackExport) {
  ...
  const content: unknown[] = [
    ...
    { text: "二、探究问题", style: "h1" }, { text: pack.input.question },
