...
export function FinalReviewPage({ taskId = "demo-tang-45m", onBack, onReturnContext, onReturnQuestion, onReturnSources, onReturnEvidence, onReturnLesson, onReturnRubric, onReturnAudit }: { taskId?: string; onBack: () => void } & ReturnActions) {
  ...
  useEffect(() => {
    if (loading || !summary.upstreamReady) return;
  ...
  const runExport = async (format: ExportFormat) => {
    ...
    try {
      const confirmed = confirmTeachingPack(draft, input); setDraft(confirmed); await saveFinalReviewDraft(storageKey, confirmed);
      await exportTeachingPack(format, { context, input, fileName: summary.fileName });
      setExportState("done"); setFeedback(`${format === "docx" ? "Word" : "PDF"} 教学包已开始下载。`);
    } catch (error) {
      console.error("Teaching pack export failed", error); setExportState("failed"); setFeedback("导出失败，未生成文件。请保留本页内容并重试。 ");
    }
  };
    ...
    <main className="context-main review-main" id="main-content" tabIndex={-1}>
      ...
      {loading ? <div className="review-skeleton" aria-busy="true"><span /><span /></div> : !summary.upstreamReady ? <section className="review-upstream-empty" tabIndex={-1}><h2>{sources.length < 4 ? "还缺少可用史料" : "设计检查尚未完成"}</h2><p>完成上游内容后，这里会自动保留已经做过的选择。</p><button className="context-primary" type="button" onClick={sources.length < 4 ? onReturnSources : onReturnAudit}>{sources.length < 4 ? "返回查找史料" : "返回设计检查"}</button></section> : <>
        <article className="delivery-preview">
          ...
          <section><div className="delivery-section-title"><span>03</span><h3>课堂活动</h3><button type="button" onClick={onReturnLesson}>✎</button></div><div className="delivery-activity-list">{input.lesson.activities.map((activity, index) => { const start = input.lesson.activities.slice(0, index).reduce((total, item) => total + item.minutes, 0); return <article key={activity.id}><span>{start}—{start + activity.minutes}</span><div><strong>{activity.title}</strong><p>{activity.studentAction}</p><small>教师引导：{activity.scaffold}</small></div></article>; })}</div></section>
        ...
        <section className="export-panel" aria-labelledby="export-heading">
          ...
          <small>文件在当前浏览器内生成并下载，不上传教学内容。建议使用 Chrome 或 Edge；Word 文件已按 Word 与 LibreOffice 通用格式生成。</small>
        </section>
      </>}
    </main>
  </WorkbenchShell>;
