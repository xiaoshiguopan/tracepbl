...
import { connectDatabase } from "@tracepbl/repositories";
import { createApp } from "./app.ts";
import { loadConfig } from "./config.ts";

const config = await loadConfig(); const database = connectDatabase(config.databaseUrl); const app = createApp(database, config);
