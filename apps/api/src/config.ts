...
export async function loadConfig(env = process.env): Promise<ApiConfig> {
  ...
  const priceProfileVersion = env.TRACEPBL_PRICE_PROFILE_VERSION ?? null;
  return ConfigSchema.parse({ host: env.TRACEPBL_API_HOST ?? "127.0.0.1", port: Number(env.TRACEPBL_API_PORT ?? 8787), allowedOrigin: env.TRACEPBL_ALLOWED_ORIGIN ?? "http://127.0.0.1:5173", databaseUrl: env.TRACEPBL_APP_DATABASE_URL ?? env.TRACEPBL_DATABASE_URL, sessionSecret, mode: env.TRACEPBL_MODE ?? "local", aiConfigured: Boolean(env.TRACEPBL_GLM_API_KEY && priceProfileVersion && env.TRACEPBL_AI_ENABLED !== "false"), urlFetchEnabled: env.TRACEPBL_URL_FETCH_ENABLED === "true", priceProfileVersion, generationReservationCnyMicros: Number(env.TRACEPBL_GENERATION_RESERVATION_CNY_MICROS ?? 0) });
}
