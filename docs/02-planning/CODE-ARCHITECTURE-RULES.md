...
| 层 | 可以做 | 不可以做 |
|---|---|---|
| `domain` | 状态机、证据门禁、权限/费用/删除宽限策略的纯逻辑 | 导入 React、Hono、LangGraph、数据库或云 SDK |
...
- PostgreSQL 是完整模式唯一数据真相；IndexedDB 仅保存当前浏览器 Demo 状态。任务菜单可以删除本机任务，P11 独立页面仍取消。
...
| 删除宽限、限额、超时 | API/Worker | 否 | 宽限固定 24 小时；限额有安全默认和上限，不能由客户端放宽 |
...
## 9. Codex Skill、插件与项目依赖边界

| 类型 | 用途 | 默认安装/连接规则 | 是否进入项目运行时 |
|---|---|---|---|
| 已有 Skill/内置工具 | 设计、审查、研究和验证方法 | 优先复用，无需重复安装 | 否 |
| GitHub Agent Skill | 补足当前设计能力 | 预览全量说明与脚本后，固定版本安装到 Codex 用户范围并验证 | 否 |
| Codex 插件 | 访问外部应用、账号或数据 | 先检查权限和依赖；连接或扩大权限需用户确认 | 否 |
| npm/数据库/云依赖 | 构成应用实现和部署 | 只能按生产依赖准入及对应编码阶段批准 | 是 |

- Skill 候选优先高星、可信来源、活跃维护、文档清楚和许可明确，但 Star 不替代内容与安全审查。
- 安装命令必须显式针对 Codex 并固定 tag 或 commit；不使用 `--force`，不自动追随最新版，不扫描隐藏目录。
- 安装前检查 Skill 是否要求执行脚本、写入仓库、访问网络、读取环境变量或操作外部系统；超出纯设计辅助所需权限时停止并报告。
- 默认不把第三方 Skill 文件提交到公开仓库。确需项目级 Skill 时，它是新的仓库产物，必须说明许可证、路径和供应链风险并单独归档。
- 插件推荐保持最小集合；未完成账号连接和权限确认时继续完成不依赖插件的工作，不制造阻塞或虚构连接状态。
- 每次设计归档记录工具名称、来源、固定版本、安装范围、用途、验证和卸载方式；Skill/插件变化不得静默改变产品架构或验收标准。
