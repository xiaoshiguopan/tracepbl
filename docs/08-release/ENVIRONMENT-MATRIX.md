...
| 环境 | 运行内容 | 数据/访问 | 外部模型 |
|---|---|---|---|
| GitHub Pages 已发布 | apps/web/dist 静态 HTML/CSS/JS/字体/最终影片；没有服务端 | 公开/合成 fixtures、预生成结果；访客各自 IndexedDB | 不调用 |
...
发布账号已核实为 xiaoshiguopan，目标为 xiaoshiguopan/tracepbl（已创建 Public）；当前静态 base 固定 /tracepbl/，必须匹配仓库名 tracepbl。其他路径需先评估，不静默改已确认路由。
