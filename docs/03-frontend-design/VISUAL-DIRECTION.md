...
> 当前修订：2026-09-02 旧 P03/P04 工作台代表图已退役；新版 P03 页面图待用户确认
...
### 9.2 P03 桌面史料阅览与选择工作台

原 `workbench-desktop-v1.svg` 是阶段 5 的历史设计证据，包含已取消的独立价值卡工作流，不再作为 P03 编码依据。新版代表页必须在用户确认后再登记为项目资产；当前产品与视觉边界以 `P03-SOURCE-DISCOVERY-DESIGN.md` 为准。

验收重点：沿用 P01 的低眩光纸张双栏和证据脉络；首屏直接阅读少量优先推荐，其后查看更多可用史料；史料内容、推荐理由、可帮助判断/不能单独证明、来源和权利限制形成稳定上下文。P04 不再成为独立页面或右侧核验表单。

### 9.3 P03 手机窄屏

原 `workbench-mobile-v1.svg` 仅保留阶段 5 历史评审价值，不再作为编码依据。新版移动重排在桌面图确认后依据同一内容顺序验证。

验收重点：单列仍保留可读史料、推荐理由、限制、来源和选择动作；没有把桌面双栏缩小；主动作不遮挡内容与焦点。
