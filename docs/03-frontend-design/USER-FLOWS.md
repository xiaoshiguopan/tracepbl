...
6. P00 的 5—7 秒混合 3D 镜头与极轻环境声均是并行增强层：王圆箓提灯进入、洞窟苏醒、流散暗示和残卷转场不承载任务信息；标题、边界、导航和开始动作从第一帧可用。滚动、键盘或导航会把导演式镜头收束到转场末帧而不被拦截。首次访问只自动运行一次，再次访问停在动态终态并可主动重看；声音被浏览器阻止时，在首次用户激活后安全尝试，失败不影响任务。

## 2. MUST-001 教学情境采集

入口 P00 → P01。教师在非阻塞敦煌首屏确认艺术/史料与演示数据边界，开始后明确进入“唐朝前期盛世”示范任务；敦煌画面不自动成为课程史料。教师填写学段、教材、课时、前置知识、阅读能力、主要困难、能力目标和产物。字段离焦做体验校验；提交时聚合错误并聚焦首错。系统生成可编辑任务简报，教师确认后进入 P02。

- 正常/成功：显示“教学情境已确认”，脉络带完成第 1 步。
- 未登录：不出现登录墙；只说明本机/临时保存边界。
- 无权限：统一进入 P12，不回显表单内容。
- 边界：0/负数/极长课时、初高中冲突、范围过大时阻止确认并给出缩小建议。
- 重复：确认中按钮显示“正在确认…”；相同请求只保留一份简报。
