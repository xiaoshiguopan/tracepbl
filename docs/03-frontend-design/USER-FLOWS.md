...
> 现役覆盖说明：游客任务管理、自由回看、P04/P10 合并、P11 取消及真实 Word/PDF 导出以 `FRONTEND-DEEP-OPTIMIZATION-SPEC.md` 为准。
...
6. P00 的 7.5 秒预渲染电影是无声增强层：笔锋吸入、河谷加速、构图匹配和山河揭示不承载任务信息；标题、边界和进入动作从第一帧可用。点击主动作只需一次并立即进入，不等待影片播完；不出现 CSS 镜头晃动、飞纸、白洗或二次确认。

## 2. MUST-001 教学情境采集

入口 P00 → P01。教师在常驻艺术/史料与演示数据边界旁一次点击进入示范任务；首页抽象山河画面不进入课程史料。P01 只填写学段、年级、教材、课次、分钟，可选课型与初步探究方向。右侧条件摘要即时更新，不需要生成。教师点击一次 `确认教学情境` 完成本步；正式问题在 P02 收束，史料条件与来源在 P03 处理，匿名学情在后续活动与支架设计中使用。
...
用户已明确取消 P11，本版本不提供应用内数据清单、单项清除或整任务清除流程。各页面继续诚实说明本机保存边界；P12 只处理不可用安全终态，不能宣称或暗示已清除数据。MUST-010 的应用内主动清除能力因此保留为明确未满足项。
