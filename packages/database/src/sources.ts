...
export class SourceRepository {
  ...
  async list(scope: WorkspaceScope, taskId: string) {
    ...
    return this.sql<Record<string, unknown>[]>`select s.id,sv.id as version_id,s.title,s.material_kind,s.canonical_url,sv.creator_or_institution,sv.source_type,sv.locator,sv.content_text,sv.rights_state,sv.verification_state,(ts.source_version_id is not null) as selected,ts.selection_order
      from core.sources s join lateral (select * from core.source_versions where source_id=s.id order by version_no desc limit 1) sv on true
      left join core.task_sources ts on ts.workspace_id=${scope.workspaceId} and ts.task_id=${taskId} and ts.source_version_id=sv.id
      where s.scope='catalog' or (s.scope='task_private' and s.workspace_id=${scope.workspaceId} and s.task_id=${taskId}) order by selected desc nulls last,ts.selection_order,s.created_at,s.id`;
  }
  async createMaterial(scope: WorkspaceScope, taskId: string, input: MaterialInput, idempotencyKey: string) {
    const requestHash = createHash("sha256").update(JSON.stringify(input)).digest("hex");
    return this.sql.begin(async (tx) => {
      const task = await tx`select 1 from core.tasks where workspace_id=${scope.workspaceId} and id=${taskId} and deleted_at is null for update`; if (!task[0]) throw new NotFoundError();
